...
async function safeFetch(raw, options = {}, depth = 0, jar = null, previousUrl = null) {
  ...
  const combinedCookie = mergeCookieHeaders(jarCookie, headers.get("cookie") || "");
  ...
  const response = await fetchWithHeaderTimeout(url, { ...options, headers, redirect: "manual" });
...
async function handleMedia(req, res, parsed) {
  try {
    ...
    const contentType = remote.headers.get("content-type") || "application/octet-stream"; const hls = /mpegurl/i.test(contentType) || /\.m3u8(?:$|\?)/i.test(target);
    res.statusCode = remote.status;
    for (const key of ["content-type","content-length","content-range","accept-ranges","etag","last-modified","cache-control"]) { const value = remote.headers.get(key); if (value) res.setHeader(key, value); }
    ...
    if (hls && remote.ok) { const rewritten = rewriteManifest(await remote.text(), remote.url || target, token); res.removeHeader("content-length"); res.setHeader("content-type","application/vnd.apple.mpegurl; charset=utf-8"); res.setHeader("cache-control","no-store"); return res.end(rewritten); }
