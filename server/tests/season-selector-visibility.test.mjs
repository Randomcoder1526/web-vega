...
assert.match(
  ...
  'blank source titles must use the shared non-empty source-label resolver',
);
const sourceFallback = await read('src/lib/playback/sourceFallback.ts');
assert.match(
  sourceFallback,
  /return `Source \$\{index \+ 1\}`/,
  'source-label resolver must retain a visible numbered fallback',
);
assert.match(
  selector,
  /value=\{String\(index\)\}/,
