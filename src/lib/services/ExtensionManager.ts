...
export class ExtensionManager {
  ...
  private isValidModuleCode(value: unknown): value is string {
  ...
  private async downloadModuleWithRetry(
    url: string,
    required: boolean,
    attempts = 3,
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        ...
        if (!this.isValidModuleCode(response.data)) {
          throw new Error("Provider module response is empty or not JavaScript");
        }
    ...
    if (required) {
      throw lastError instanceof Error
        ? lastError
        : new Error(`Failed to download provider module: ${url}`);
    }
    return undefined;
  }
  ...
  async downloadProviderModules(
    ...
    try {
      // catalog is optional in the current provider build contract. Some
      // search-only providers only ship posts/meta/stream.
      const requiredFiles = ["posts", "meta", "stream"];
      const optionalFiles = ["catalog", "episodes", "settings"];
      const allFiles = [...requiredFiles, ...optionalFiles];
      const cacheBust = encodeURIComponent(version || String(Date.now()));

      const modules: Record<string, string> = {};
      await Promise.all(
        allFiles.map(async (fileName) => {
          const url = `${sourceUrl}/dist/${providerValue}/${fileName}.js?v=${cacheBust}`;
          console.log(`Downloading: ${url}`);
          const required = requiredFiles.includes(fileName);
          const code = await this.downloadModuleWithRetry(url, required);
          if (code) {
      ...
      // Commit only after the complete replacement has validated. This makes
      // installs/updates transactional from the user's point of view.
    ...
    try {
      ...
      await Promise.all(
        allFiles.map(async (fileName) => {
          ...
          const required = requiredFiles.includes(fileName);
          const code = await this.downloadModuleWithRetry(fileUrl, required);
          if (code) modules[fileName] = code;
        }),
      );
