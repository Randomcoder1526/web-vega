...
import {
  ...
  shouldRetryPlaybackFailure,
} from "../playback/playbackErrors";
import { reportHandledIssue, reportUnexpectedIssue } from "../errors/errorReporter";
import { extractBlockedHost } from "../playback/hostBlockCircuit";
import { parseQualityHeight } from "../playback/sourceFallback";
...
interface UseStreamOptions {
  ...
  routeParams?: StreamRouteParams | null;
  provider: string;
  enabled?: boolean;
}
...
const PROVIDER_STREAM_TIMEOUT_MS = 30_000;

const getProviderContentType = (
  routeParams: StreamRouteParams | null | undefined,
): string => {
  return routeParams?.type?.trim() || "movie";
};

const withProviderStreamTimeout = async <T>(
  ...
  try {
    ...
    if (timedOut) {
      const timeoutError = new Error(
        `Provider stream request timed out after ${PROVIDER_STREAM_TIMEOUT_MS}ms`,
...
const getCompletedDownload = (
  activeEpisode: StreamEpisode | null | undefined,
  routeParams: StreamRouteParams | null | undefined,
) => {
...
export const useStream = ({
  ...
  const providerContentType = getProviderContentType(routeParams);
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      ...
      if (import.meta.env.DEV) {
        console.debug("Fetching stream for:", activeEpisode);
      }
      ...
      // Fetch streams from provider with a bounded timeout. React Query's
      // cancellation signal is forwarded so navigation/source switches abort
      // the provider worker immediately rather than leaving stale extraction work.
      let data: Stream[] = [];
      try {
        data =
          (await withProviderStreamTimeout(signal, (requestSignal) =>
    ...
    retry: (failureCount, error) => shouldRetryPlaybackFailure(
  ...
  useEffect(() => {
    if (streamData && streamData.length > 0) {
      setSelectedStream((prev) => {
        // A user may have manually picked a server. Keep that choice when the
        // link is still present in the refreshed list instead of being forced
        // back to the first entry by a refetch/retry.
        if (
          prev?.link &&
          streamData.some((stream) => stream.link === prev.link)
        ) {
          return prev;
        }
        return streamData[0];
  ...
  const streamFailure = useMemo(
    () => (error ? classifyPlaybackError(error) : null),
    [error],
  );

  // Handle errors once with a normalized category. The player uses the same
  // object to decide whether it should retry, switch streams, or surface a
  // terminal host-block/dead-source message.
  useEffect(() => {
    if (!error || !streamFailure) return;

    if (streamFailure.category === "unknown") {
      reportUnexpectedIssue("stream", error);
      return;
    }

    const blockedHost =
      streamFailure.category === "forbidden" ? extractBlockedHost(error) : null;

    reportHandledIssue({
      scope: "stream",
      category: streamFailure.category,
      userMessage: streamFailure.userMessage,
      error,
      status: streamFailure.status,
      dedupeKey: blockedHost
        ? `${routeParams?.providerValue || provider || "unknown"}:${blockedHost}`
        : undefined,
      dedupeWindowMs: blockedHost ? 60_000 : undefined,
      details: blockedHost ? { host: blockedHost } : undefined,
    });
  }, [error, provider, routeParams?.providerValue, streamFailure]);

  const retryCurrentExtraction = () => refetch();
...
interface UseFallbackAudioStreamOptions {
  fallbackCandidate?: StreamEpisode | null;
  routeParams?: StreamRouteParams | null;
  provider: string;
  enabled?: boolean;
}

export const useFallbackAudioStream = ({
  fallbackCandidate,
  routeParams,
  provider,
  enabled = true,
}: UseFallbackAudioStreamOptions) => {
  const providerContentType = getProviderContentType(routeParams);
  const qualityHeight = parseQualityHeight(fallbackCandidate?.quality);
  const { data: fallbackStreams = EMPTY_STREAMS, isLoading, error } = useQuery<
    Stream[],
    Error
  >({
    queryKey: [
      "fallback-audio-stream",
      fallbackCandidate?.link,
      qualityHeight,
      providerContentType,
      routeParams?.providerValue || provider,
    ],
    queryFn: async ({ signal }) => {
      if (!fallbackCandidate?.link) return [];
      const data =
        (await withProviderStreamTimeout(signal, (requestSignal) =>
          providerManager.getStream({
      ...
      const inheritedQuality =
        qualityHeight !== null ? String(qualityHeight) : fallbackCandidate.quality;
      return data.map((stream) => ({
        ...stream,
        quality: inheritedQuality || stream.quality,
      })) as Stream[];
    },
    enabled: enabled && !!fallbackCandidate?.link,
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
    retry: (failureCount, queryError) =>
      shouldRetryPlaybackFailure(classifyPlaybackError(queryError), failureCount),
    retryDelay: (attemptIndex) => Math.min(1000 * 2 ** attemptIndex, 10000),
...
export const useVideoSettings = () => {
  ...
  const processVideoTracks = (tracks: any[]) => {
    ...
    if (import.meta.env.DEV) {
      console.debug("Processing video tracks:", uniqueTracks);
    }
