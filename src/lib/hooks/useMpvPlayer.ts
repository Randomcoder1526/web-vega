...
import { classifyPlaybackError, type PlaybackFailure } from "../playback/playbackErrors";
import { isTauriRuntime } from "../../platform/runtime";

// Per-load diagnostics belong in dev builds only; production keeps the console
// clean and avoids the noise printing auth-prefixed stream URLs can create.
const mpvDebugLog = (...args: unknown[]) => {
  if (import.meta.env.DEV) console.log("[MPV]", ...args);
};
...
export interface UseMpvPlayerOptions {
  ...
  onFallbackAudioFailure?: (reason?: string) => void;
}
...
export const useMpvPlayer = (opts?: UseMpvPlayerOptions) => {
  ...
  const initPlayer = useCallback(async () => {
    ...
    try {
      ...
      const unlistenEvt = await listenEvents((event) => {
        ...
        if (event.event === "web-backup-audio-failure") {
          optsRef.current?.onFallbackAudioFailure?.((event as any).reason);
        }
        if (event.event === "web-playback-error") {
          const message = (event as any).text || "This stream could not be played by the browser.";
          const failure = classifyPlaybackError({
            message,
            status: (event as any).status,
            category: (event as any).category,
          });
          setIsBuffering(false);
          if (optsRef.current?.onPlaybackFailure) {
            optsRef.current.onPlaybackFailure(failure);
          } else {
            setInitializationError(`Browser playback failed: ${failure.userMessage}`);
          }
        }
  ...
  const destroyPlayer = useCallback(() => {
    ...
    // Web runtime: the 500ms destroy debounce must not leave the hidden
    // fallback-audio element playing after navigating away (Back button,
    // route change). Stop the underlying media immediately.
  ...
  const loadFile = useCallback(
    async (
      ...
      if (!(window as any).__TAURI_INTERNALS__) {
        setInitializationError(null);
        await setProperty("vega-stream-type", type || "").catch(() => {});
        await setProperty("vega-backup-audio-url", fallbackAudio?.url || "").catch(() => {});
        await setProperty("vega-backup-audio-type", fallbackAudio?.type || "").catch(() => {});
        await setProperty(
          "vega-backup-audio-label",
          fallbackAudio?.quality ? `Fallback Audio ${fallbackAudio.quality}p` : "Fallback Audio",
        ).catch(() => {});
        await setProperty(
          "vega-backup-audio-headers",
          JSON.stringify(fallbackAudio?.headers || {}),
        ).catch(() => {});
      }
      setTracks([]);
      setChapters([]);
      ...
      try {
        ...
        if (type === "torrent" || url.startsWith("magnet:")) {
          try {
            mpvDebugLog("entering torrent block");
            if (!isTauriRuntime()) {
              throw new Error(
                "Torrent streaming is only available in the Vega desktop app. Launch the desktop build to play this source.",
              );
            }
            const { invoke } = await import("@tauri-apps/api/core");
            const apiPort = await invoke<number>("get_torrent_api_port");
            mpvDebugLog("get_torrent_api_port returned:", apiPort);
            if (apiPort) {
              currentActiveTorrentApiPort = apiPort;
              mpvDebugLog("api port:", apiPort);

              let infoHash = currentActiveTorrentInfoHash;
              let torrentFiles: any[] = [];
              if (currentActiveTorrentUrl !== url || !infoHash) {
                mpvDebugLog("sending POST /torrents for:", url);
                ...
                mpvDebugLog("POST /torrents resolved");
              ...
              finalUrl = streamUrl;
              mpvDebugLog("Streaming file", fileId, finalUrl);
              isProxied = true;
              isTorrent = true;
            } else {
              mpvDebugLog("apiPort is falsy!");
            }
          } catch (e: any) {
            if (e.name === "AbortError" || e.message === "Aborted") {
              mpvDebugLog("Stream fetch aborted");
          ...
          try {
            ...
            mpvDebugLog("proxy port:", proxyPort);
        ...
        if (err.name === "AbortError" || err.message === "Aborted") {
          mpvDebugLog("Aborted previous load");
          return;
        }
        if (!(window as any).__TAURI_INTERNALS__) {
  ...
  return {
    ...
    getProperty: async (prop: string) => {
      if (isInitialized) {
        try {
          return await getProperty(prop, "native");
        } catch (e) {}
      }
      return undefined;
    },
