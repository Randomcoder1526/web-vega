...
const parseRawGithubUrl = (url: URL): ParsedGithubSource | null => {
  if (url.hostname !== RAW_GITHUB_HOST) return null;
  const segments = url.pathname.split('/').filter(Boolean);
  if (segments.length < 4) return null;
  const author = segments[0];
  const repo = segments[1];
  let branch = DEFAULT_BRANCH;
  if (segments[2] === 'refs' && segments[3] === 'heads' && segments.length > 4) {
...
export const createProviderSource = (value: string): ProviderSource => {
  ...
  if (!input) throw new Error('Provider source value is required');

  const isUrlInput = /^https?:\/\//i.test(input);

  if (isUrlInput) {
    let parsed: URL;
    try { parsed = new URL(input); } catch { throw new Error('Invalid provider source URL'); }

    const parsedSource = parseRawGithubUrl(parsed) || parseGithubRepoUrl(parsed);
    if (!parsedSource) throw new Error('Only github.com or raw.githubusercontent.com URLs are supported');

    return {
      author: parsedSource.author,
      url: buildRawGithubUrl(parsedSource.author, parsedSource.repo, parsedSource.branch),
      isDefault: false,
    };
  }

  const author = input.replace(/^@/, '').trim();
  if (!author) throw new Error('Invalid GitHub author name');
