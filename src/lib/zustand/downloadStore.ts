  ...
  persist(
    (set, get) => {
      const startDownloadItem = async (id: string) => {
        ...
        // Zustand updates synchronously, but refresh the item after the state
        // transition so any concurrent metadata changes are not lost.
        item = get().downloads[id];
...
export function initDownloadListeners() {
  ...
  // Torrent polling requires the native desktop torrent backend.
  if (isTauriRuntime()) {
    startTorrentPolling();
  }
}
