...
import { buildMovieExtractionQueue } from "../lib/playback/sourceFallback";
...
export const MetaPage: React.FC = () => {
  ...
  const play = (items: Array<{ title: string; link: string }>, index: number, type: string) => {
    ...
    const playbackItems =
      type === "movie"
        ? buildMovieExtractionQueue(filteredLinkList, selectedItem)
        : items;
    const playbackIndex = type === "movie" ? 0 : index;
    const selectedSourceIndex = Math.max(
      filteredLinkList.findIndex((source: Link) =>
        source === activeSeason ||
        source.title === activeSeason?.title ||
        source.directLinks?.some((item: NonNullable<Link["directLinks"]>[number]) => item.link === selectedItem?.link),
      ),
      0,
    );
    ...
    navigate("/player", {
      state: {
        episodeList: playbackItems,
        linkIndex: playbackIndex,
        sourceGroups: filteredLinkList,
  ...
  const showSeasonSelector = filteredLinkList.length > 1;
