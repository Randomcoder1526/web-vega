...
import { resume } from "@noriginmedia/norigin-spatial-navigation";
import {
  LuBlocks as Blocks,
  LuCheck as Check,
...
import { getProviderWebCompatibility } from "../lib/providers/webCompatibility";
import {
  exportProviderHealth,
  getProviderDiagnostic,
  testProviderCatalogAndPosts,
  type ProviderDiagnostic,
} from "../lib/providers/providerDiagnostics";
...
const ProviderIdentity = ({ provider }: { provider: ProviderExtension }) => {
  const providerModule = extensionStorage.getProviderModules(
    provider.value,
    provider.source?.author,
  );
  const compatibility = getProviderWebCompatibility(provider, providerModule);
    ...
    <div className="provider-identity">
      ...
      <div className="provider-copy">
        ...
        <div className="provider-metadata">
          ...
          <span
            className={`provider-chip compatibility ${compatibility.level}`}
...
export const ExtensionsPage: React.FC = () => {
  ...
  const [healthResults, setHealthResults] = useState<Record<string, ProviderDiagnostic>>({});
  ...
  const runHealthCheck = async (provider: ProviderExtension) => {
    const key = providerKey(provider);
    setHealthKey(key);
    try {
      const diagnostic = await testProviderCatalogAndPosts(provider);
      setHealthResults((current) => ({ ...current, [key]: diagnostic }));
    } catch {
      setError("Provider test could not start. Please check the browser console.");
    } finally {
      setHealthKey(null);
    }
  };
  ...
  const downloadHealthReport = () => {
    const data = exportProviderHealth(installedProviders);
    const objectUrl = URL.createObjectURL(new Blob([data], { type: "application/json" }));
    const anchor = document.createElement("a");
    anchor.href = objectUrl;
    anchor.download = "vega-provider-health.json";
    ...
    <main className="extensions-page">
      ...
      <section className="providers-section" aria-labelledby="providers-title">
        ...
        {providers.length === 0 ? (
          ...
          <>
          <div className="provider-health-toolbar">
            <div>
              <strong>Provider health</strong>
              <p>Run a catalog/posts check for an installed provider. This does not test streams or playback.</p>
            </div>
            <FocusableButton
              className="provider-health-export"
              onClick={downloadHealthReport}
              disabled={installedProviders.length === 0}
              title="Export a privacy-safe provider health report"
            >
              Export report
            </FocusableButton>
          </div>
          <div className="providers-list">
            {providers.map((provider) => {
              ...
              const diagnostic = healthResults[key] || getProviderDiagnostic(provider);
              const diagnosticsCurrent = diagnostic?.version === provider.version ? diagnostic : null;
              ...
              return (
                ...
                >
                  ...
                  >
                    {installed ? (
                      <>
                        ...
                        <FocusableButton
                          className="provider-health-button"
                          onClick={() => void runHealthCheck(provider)}
                          disabled={healthKey !== null}
                          title={`Check ${provider.display_name} catalog and posts (no playback)`}
                          focusKey={`PROVIDER_CHECK_${key}`}
                        >
                          {healthKey === key ? "Checking..." : "Test"}
                        </FocusableButton>
                  ...
                  {installed && diagnosticsCurrent && (
                    <div className={`provider-health-result ${diagnosticsCurrent.posts}`}>
                      <strong>Posts: {diagnosticsCurrent.posts}</strong>
                      <span> · {diagnosticsCurrent.postsFound} items · {new Date(diagnosticsCurrent.checkedAt).toLocaleString()}</span>
                      <p>{diagnosticsCurrent.reason}</p>
                    </div>
                  )}
                </article>
              );
