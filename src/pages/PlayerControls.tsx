...
import { getSourceDisplayName } from "../lib/playback/sourceFallback";
...
interface PlayerControlsProps {
  ...
  onSelectSource?: (index: number) => void | Promise<void>;
...
export const PlayerControls: React.FC<PlayerControlsProps> = ({
  ...
  useEffect(() => {
    const interactingWithPopup = Boolean(openMenu || showOnlineSearch);
    onScrubbingChange?.(interactingWithPopup);
    return () => {
      if (interactingWithPopup) onScrubbingChange?.(false);
    };
  }, [openMenu, showOnlineSearch, onScrubbingChange]);
  ...
  const handleTrackPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const onMove = (ev: PointerEvent) => updateTimelinePreview(ev);
      const finish = (ev: PointerEvent, shouldSeek: boolean) => {
        const seekTime = updateTimelinePreview(ev);
        if (shouldSeek && seekTime !== null) onSeek(seekTime);
      ...
      <div className="player-bottom-bar">
        <div className="player-timeline" onClick={stop} onDoubleClick={stop}>
          ...
          <div
            ...
            onMouseEnter={(event) => {
              // Hovering alone only previews the seek position. Scrubbing is
              // reserved for an actual drag (pointerdown), so parking the
              // pointer over the timeline no longer pins the controls open.
        ...
        <div className="player-actions-row">
          ...
          >
            ...
            {sourceData.length > 1 && (
              <div className="inline-menu-container">
                <button
                  className={`action-btn text-btn ${openMenu === "source" ? "active" : ""}`}
                  onClick={(e) => toggleMenu(e, "source")}
                  title="Change source"
                  aria-label="Change playback source"
                >
                  <SourceIcon size={20} />
                  <span>
                    {getSourceDisplayName(
                      sourceData[selectedSourceIndex],
                      selectedSourceIndex,
                    )}
                  </span>
                </button>
                {openMenu === "source" && (
                  <div className="inline-menu right wide" onClick={stop}>
                    {sourceData.map((source: any, idx: number) => (
                      <button
                        key={`${idx}:${source?.title || source?.episodesLink || "source"}`}
                        className={`inline-menu-item ${selectedSourceIndex === idx ? "selected" : ""}`}
                        onClick={() => {
                          void onSelectSource?.(idx);
                          setOpenMenu(null);
                        }}
                      >
                        <div className="track-details">
                          <span className="track-name">
                            {getSourceDisplayName(source, idx)}
                          </span>
                          {source?.quality && (
                            <span className="track-lang">{source.quality}</span>
                          )}
                        </div>
                        {selectedSourceIndex === idx && <Check size={14} />}
                      </button>
                    ))}
                  </div>
                )}
