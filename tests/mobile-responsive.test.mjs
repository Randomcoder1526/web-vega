...
test('mobile layout replaces the desktop sidebar with a bottom navigation bar', () => {
  ...
  assert.match(css, /\.sidebar-nav[\s\S]*grid-template-columns:\s*repeat\(5/);
...
test('mobile player action bar remains reachable horizontally and extension dialogs stay above the keyboard', () => {
  const css = read('../src/styles/responsive.css');
  assert.match(css, /\.player-actions-row[\s\S]*overflow-x:\s*auto/);
  assert.match(css, /\.player-actions-row[\s\S]*scrollbar-width:\s*none/);
  assert.match(css, /\.extensions-dialog-content[\s\S]*top:\s*max\(/);
  assert.match(css, /\.extensions-dialog-actions[\s\S]*position:\s*sticky/);
